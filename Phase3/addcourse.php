...
<br><br><br><br>
 ...
 <?php
 session_start();
 echo 'Enrooling course for student with email id = '.$_SESSION['login_user'];
 $myusername=$_SESSION['login_user'];
	...
	$con=mysqli_connect("localhost","root","root","studlearning");
		...
		$sql3="SELECT name FROM courses WHERE course_id NOT IN (SELECT course_id FROM course_details WHERE usn='$usn') ";
		$result3=mysqli_query($con,$sql3);
		
        // Associative array
        //$row2=mysqli_fetch_assoc($result3);
		echo'<form action="finish_adding.php" method="post">';
        foreach($result3 as $row2)
        { 
          $value=$row2['name'];
		  echo "<input type='checkbox' name='course[]' value='$value'>";
		  printf (" %s\n",$row2["name"]);
		  echo'<br>';
